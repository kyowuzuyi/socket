...
//	session_start();
	...
	$membercount->bindValue(':eventURL',$eventURL);
	$membercount->execute();
	$memberno=$membercount->rowCount();
	
	$commentRow = $membercount->fetchAll(PDO::FETCH_BOTH);
...
<h1>イベントの確認</h1>
	<div>
    	<p>イベント名：<span><?php echo $row['eventname']; ?></p></span>
        <p>時間：<span><?php echo $row['eventtime'] ?></p></span>
        <p>場所：<span><?php echo $row['eventplace'] ?></p></span>        
        <p>メモ：<span><?php echo $row['eventmemo'] ?></p></span> 
</div>
...
<br>
    ...
    <input type="hidden" name="eventURL" value="<?php echo $row['eventURL']; ?>">
